...
class AnimationEntity{
	...
	AnimationEntity(String animationSpritePath, Entity caster, int width, int height,int numberOfFrames, boolean noRotation) {
		...
		if(caster instanceof Character) {
			...
			if(noRotation) setPositionAccordingToDirection(caster.getCurrentEntityPosition());
			else setPositionAccordingToDirectionWithRotation(caster.getCurrentEntityPosition());
		}else {
	...
	private void setPositionAccordingToDirectionWithRotation(Vector2 casterPosition) {
		switch(direction) {
	...
	private void setPositionAccordingToDirection(Vector2 casterPosition) {
		switch(direction) {
			case RIGHT: currentAnimationPosition=new Vector2(casterPosition).add(0.5f, -0.5f);break;
			case UP: currentAnimationPosition=new Vector2(casterPosition).add(-0.5f, 0.5f);break;
			case LEFT:currentAnimationPosition=new Vector2(casterPosition).add(-1.5f,-0.5f);break;
			case DOWN:currentAnimationPosition=new Vector2(casterPosition).add(-0.5f,-1.5f);break;
		}
	}
